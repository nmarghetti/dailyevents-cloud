...
function refreshStatuses(a){var b=ui.labels.participants;b.empty();b.append($("<li/>",{"data-role":"list-divider",text:"Attending today"}));for(var d in a)"yes"==a[d].reply&&b.append($("<li/>",{text:a[d].participant}));b.listview("refresh")}
function refreshComments(a){var b=ui.labels.comments;b.empty();b.append($("<li/>",{"data-role":"list-divider",text:"Comments"}));for(var d in a){var c=new Date(parseInt(a[d].timestamp)),c=toTwoDigits(c.getHours())+":"+toTwoDigits(c.getMinutes())+" "+a[d].participant,c=$("<li/>",{"data-icon":"false",html:c+": "+linkify(a[d].comment)});c.addClass("ui-li-static");b.append(c)}b.listview("refresh")}
function linkify(a){return a.replace(/(\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/ig,'<a href="$1">$1</a>')}function confirmAttendance(){setStatus("yes")}function cancelAttendance(){setStatus("no")}
