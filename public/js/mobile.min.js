...
function setStatus(a){data.groupId&&(disableButtons([ui.buttons.reply_yes,ui.buttons.reply_no]),Parse.Cloud.run("setStatus",{groupId:data.groupId,participant:getDisplayName(),reply:a,timestamp:getTimestamp(),timezone:getTimezone()},{success:function(){refreshEvent();enableButtons([ui.buttons.reply_yes,ui.buttons.reply_no])},error:function(){enableButtons([ui.buttons.reply_yes,ui.buttons.reply_no])}}))}
function addComment(){var a=ui.fields.comment.val();data.groupId&&a&&(disableButtons([ui.buttons.add_comment]),Parse.Cloud.run("addComment",{groupId:data.groupId,participant:getDisplayName(),comment:a,timestamp:getTimestamp(),timezone:getTimezone()},{success:function(){ui.fields.comment.val("");refreshEvent();enableButtons([ui.buttons.add_comment])},error:function(){enableButtons([ui.buttons.add_comment])}}))}
